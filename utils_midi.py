...
def frec_a_midi(frec):
    """pasa una frecuencia a la nota midi mas cercana """
...
def exportar_melodia_a_midi(melodia, nombre_archivo="mensaje.mid", bpm=60, instrumento=0):
    ...
    ##elegir instrumento  y duracion de la nota
    track.append(Message('program_change', program=instrumento, time=0))
    track.append(MetaMessage('set_tempo', tempo=bpm2tempo(bpm), time=0))

    mel_orden = sorted(melodia, key=lambda x: (x[0],x[1])) # num_compas y beat_compas ordenados

    for (compas, beat, frec, es_msj) in mel_orden:
        nota_midi = frec_a_midi(frec)
        if es_msj:
            duration_ticks = 480 # negra (1seg)
            velocity = 60
        else:
            duration_ticks = 240 # corchea 0.5 seg
            velocity = 70
    ...
    #
    cierre = [60, 64, 48]   # acorde final C4, E4, C3 
    for nota in cierre:
        track.append(Message('note_on', note=nota, velocity=80, time=0))
    track.append(Message('note_off', note=cierre[0], velocity=70, time=480))
    track.append(Message('note_off', note=cierre[1], velocity=80, time=480))
    track.append(Message('note_off', note=cierre[2], velocity=80, time=480))

    mid.save(nombre_archivo)
    #print(f" midi guardado como: {nombre_archivo}")
